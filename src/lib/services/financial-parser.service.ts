...
2. ถ้าใช่ ให้ extract ข้อมูลดังนี้:
   ...
   - ⚠️ "รวมค่าหมู", "ค่าหมูทั้งหมด", "หมูทั้งหมด" = ดึงจำนวนหมูจากวันก่อน (carry pork qty)
...
function applyDeterministicEnrichment(parsed: ParsedFinancialInput, text: string): void {
  ...
  if (wantsIncludePork(text)) {
    parsed.note = parsed.note?.trim() ? `${parsed.note.trim()} · รวมค่าหมู` : "รวมค่าหมู";
    parsed.isFinancialData = true;
...
/** User wants pork qty/prices carried from the prior record (no per-kg amounts in message). */
export const INCLUDE_PORK_RE =
  /รวม(?:ค่า)?หมู(?:ทั้งหมด)?|ค่าหมู(?:ทั้งหมด)?|หมูทั้งหมด|คิดหมู(?:ด้วย)?|รวมหมู(?:ด้วย)?/i;

export function wantsIncludePork(text: string): boolean {
  return INCLUDE_PORK_RE.test(text);
}

export function parseShopFollowUp(text: string): {
  shop: { shopId: string; shopName: string };
  includePork: boolean;
} | null {
  const shop = detectShopFromText(text);
  const includePork = wantsIncludePork(text);
  if (!shop || !includePork) return null;
  return { shop, includePork };
}
...
export function parseFinancialMessageWithRegex(text: string): ParsedFinancialInput {
  ...
  parsed.isFinancialData =
    ...
    wantsIncludePork(text) || shopping !== null;
...
export function looksLikeFinancialData(text: string): boolean {
  const patterns = [
    ...
    /(?:^|\n)[\u0E00-\u0E7Fa-zA-Z][^\n\d]{1,30}\s+\d{2,}(?:\s*\([\d.]+\))?/m,
    // Branch + include pork instruction (no amounts required)
    /(?:ตลาดญี่ปุ่น|ญี่ปุ่น|หนองป(?:ิง|ล(?:ิง|ั่ง|ัง))|ป(?:ิง|ล(?:ิง|ั่ง|ัง)))[\s\S]*(?:รวม(?:ค่า)?หมู|ค่าหมู(?:ทั้งหมด)?|หมูทั้งหมด)/i,
    INCLUDE_PORK_RE,
