...
2. ถ้าใช่ ให้ extract ข้อมูลดังนี้:
   ...
   - ⚠️ "รวมค่าหมู" = ให้รวมค่าหมูจากวันก่อน (carry pork qty)
...
export function wantsIncludePork(text: string): boolean {
  return /รวม(?:ค่า)?หมู|คิดหมู(?:ด้วย)?|รวมหมู(?:ด้วย)?/i.test(text);
}
...
export function looksLikeFinancialData(text: string): boolean {
  const patterns = [
    ...
    /(?:^|\n)[\u0E00-\u0E7Fa-zA-Z][^\n\d]{1,30}\s+\d{2,}(?:\s*\([\d.]+\))?/m,
    // Branch + include pork instruction
    /(?:หนองป(?:ิง|ล(?:ิง|ั่ง))|ป(?:ิง|ล(?:ิง|ั่ง)))[\s\S]*รวม(?:ค่า)?หมู/i,
    /รวม(?:ค่า)?หมู/i,
