...
2. ถ้าใช่ ให้ extract ข้อมูลดังนี้:
   ...
   - extraExpenses: รายการค่าใช้จ่ายอื่นๆ [{name, amount}] เช่น "ซื้อของ แม็คโคร 1220" → {name:"ซื้อของ แม็คโคร", amount:1220} (ถ้ามีชื่อสาขานำหน้า ให้แยกชื่อสาขาออกก่อน)
...
function parseFinancialMessageWithRegex(text: string): ParsedFinancialInput {
  ...
  for (const rawLine of text.split(/\n/).map((l) => l.trim()).filter(Boolean)) {
    ...
    if (/^(?:รายรับ|รายได้|\+)\s*.+?\s+[\d,]+$/.test(rawLine)) continue;
    // Drop the branch name so "หนองปิง ซื้อของ แม็คโคร 1220" still yields the expense
    const line = rawLine.replace(SHOP_KW, "").trim();
    if (!line) continue;
